# Task 3 — `tui::style_map` (cchud → ratatui mapping)

**Цель:** Тонкий мост между cchud типами рендера и ratatui. Чистая функция `to_span(&StyledSegment) -> ratatui::text::Span` плюс хелперы `to_ratatui_color`, `to_ratatui_style`. Используется из T9 (`panels::preview`) и T14 (TUI snapshot tests).

**Files:**
- Modify: `src/tui/style_map.rs` — наполнить (был skeleton после T1)

---

- [ ] **Step 1: Написать failing test**

В `src/tui/style_map.rs`:
...
use crate::render::{Color, StyledSegment, Style};
...
/// Converts cchud `Style` → ratatui `Style`. fg/bg + bold/italic/dim/underline.
/// `Style::adapt(level)` НЕ вызывается — preview всегда forces TrueColor через
/// `Renderer::for_preview` (T2). Если fg/bg = None — соответствующее поле в ratatui тоже None.
#[must_use]
...
```bash
cargo test --features tui --locked tui::style_map
```

Expected: 6 PASS.

- [ ] **Step 3: Verify --no-default-features build не сломан**

```bash
cargo build --locked --no-default-features
```

Expected: PASS (без feature `tui` модуль исключён, `style_map` не компилируется).
...
```bash
git add src/tui/style_map.rs
git commit -m "$(cat <<'EOF'
feat(phase-8): T3 style_map — cchud Style/Color → ratatui Style/Color/Span

- to_ratatui_color: Rgb → Rgb, Ansi256 → Indexed
- to_ratatui_style: fg/bg + Modifier::{BOLD,ITALIC,DIM,UNDERLINED}
- to_span: StyledSegment → ratatui::text::Span<'static>; hyperlink dropped (no OSC 8 in TUI)
- 6 unit-tests covering color mapping, modifier composition, and lifetime
