# Task 8 — widgets/usage.rs — 4 виджета

**Цель:** Реализовать 4 usage-виджета (`SessionUsage`, `WeeklyUsage`, `BlockResetTimer`, `WeeklyResetTimer`) поверх `payload.rate_limits.{five_hour, seven_day}`. Привязать в `build_one`. ≥8 unit-тестов с зафиксированным `now_ms`.

**Files:**
- Modify: `src/widgets/usage.rs` — 4 виджета
- Modify: `src/widgets/mod.rs::build_one` — заменить stub на реальные привязки для 4 вариантов

---

- [ ] **Step 1: Write failing tests**

В `src/widgets/usage.rs`:
...
Run: `cargo test --lib widgets::usage`
Expected: FAIL.

- [ ] **Step 3: Реализовать виджеты**

В `src/widgets/usage.rs` (после struct-определений):

```rust
fn buckets(ctx: &RenderContext<'_>) -> Option<&RateLimits> {
...
В `src/widgets/mod.rs::build_one` заменить временный stub-блок для usage-вариантов:
...
(Оставить остальные Phase 7 варианты `ClaudeAccountEmail`/`FreeMemory`/`Skills`/`AlignRight` в stub-блоке до T9/T10/T11.)
...
```bash
git add src/widgets/usage.rs src/widgets/mod.rs
git commit -m "feat(phase-7): T8 — usage cluster (SessionUsage/WeeklyUsage/BlockReset/WeeklyReset)"
```
